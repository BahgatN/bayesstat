...
$$f(x_1,x_2,...,x_n|\theta) = \prod_{i=1}^{i=n}f(x_i|\theta) =  \prod_{i=1}^{i=n} \frac{1}{y_{i}!}\theta^{y_{i}}e^{-\theta}$$
\newline 
For now we will work with a class of conjugate prior distributions that will
make posterior calculations simple.A class of prior densities is
conjugate for a sampling model p(y1, . . . , yn|) if the posterior distribution is
...
to include terms like c1e−c2 for numbers c1 and c2. The simplest class of
